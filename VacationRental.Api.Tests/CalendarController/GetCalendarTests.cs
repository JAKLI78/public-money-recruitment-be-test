...
public class GetCalendarTests
{
    ...
    [Fact]
    public async Task CorrectRequest_ReturnsCalculatedCalendar()
    {
    ...
    [Fact]
    public async Task NotExistingRental_ReturnsNotFoundMessage()
    {
        using var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={5}&start=2000-01-01&nights=5");

        getCalendarResponse.IsSuccessStatusCode.Should().BeFalse();
        getCalendarResponse.StatusCode.Should().Be(HttpStatusCode.NotFound);

        var errorDetails = await getCalendarResponse.Content.ReadFromJsonAsync<ErrorDetails>();

        errorDetails.Message.Should().Be("Rental with such id not found.");
    }

    [Fact]
    public async Task NegativeNights_ReturnsBadRequestWithMEssage()
    {
        using var getCalendarResponse = await _client.GetAsync($"/api/v1/calendar?rentalId={5}&start=2000-01-01&nights=-5");

        getCalendarResponse.IsSuccessStatusCode.Should().BeFalse();
        getCalendarResponse.StatusCode.Should().Be(HttpStatusCode.BadRequest);

        var errorDetails = await getCalendarResponse.Content.ReadFromJsonAsync<ErrorDetails>();

        errorDetails.Message.Should().Be("Nights must be positive");
    }
}